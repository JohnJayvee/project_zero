...
function get_where_section($table_name, $id)
{
	$conn = getConnection();
	$sql = "SELECT * FROM $table_name where section_id=$id";
	$result = $conn->query($sql);
	return $result;
}
...
function count_attendance($setName,$id)
{
	
	$conn = getConnection();
	$sql = "SELECT COUNT(*) FROM attendance_records WHERE studid='$id' AND attendance_status='$setName'";
	$result = $conn->query($sql);
...
function get_join_date($table_name1,$table_name2, $column,$value)
{
	...
	$conn = getConnection();
	$sql="SELECT * FROM $table_name1 JOIN $table_name2 ON $table_name1.".$column."=$table_name2.".$column." WHERE $table_name1.section_id=$value AND date(datetoday)='$datetoday'"; 
...
function get_section_from_id($table_name,$id)
{
	$conn = getConnection();
	$sql = "SELECT * FROM $table_name where section_id=$id";
	$result = $conn->query($sql);
